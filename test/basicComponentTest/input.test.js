...
const createJSXForInput = ({ displayString, uniqueId }) => {
  return <Input displayString={displayString} uniqueId={uniqueId} />;
};
...
describe("test Input component for properties", () => {
  ...
  test.each(inputTypes)(
    "check if Input properties are correctly included",
    ({ displayString, uniqueId }, done) => {
      const testInput = create(
      ...
      expect(testInput.rendered.props.placeholder).toBe(
