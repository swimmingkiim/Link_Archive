...
describe("test Select component", () => {
  ...
  test("test if the options are correct", (done) => {
    ...
    expect(testSelect.children[0].props["data-filter-type"]).toBe("tag");
  ...
  test("test Select dropdown when you click it", (done) => {
    ...
    while (numberOfTest) {
      ...
      expect(
        document.querySelector("#selected-filter").classList.contains("down")
      ).toBe(true);
      document.querySelector("#selected-filter").click();
      expect(
        document.querySelector("#selected-filter").classList.contains("down")
  ...
  test("test if option clicked, check the values changed", (done) => {
    ...
    while (numberOfTest < 4) {
      ...
      expect(selectedValue.dataset.filterType).toBe(
        ulList[numberOfTest].dataset.value
      );
      numberOfTest++;
    }
    done();
